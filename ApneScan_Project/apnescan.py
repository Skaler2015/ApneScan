...
# calls directly are imported back; the rest are internal helpers.
from apnescan_lib.naming_engine import (
...
from apnescan_lib.fingerprint import (
    _dm_keywords, _dm_text_hash, _dm_jaccard, _dm_text_sim, _dm_ahash,
    _dm_dhash, _dm_phash, _dm_hash_sim, _dm_decode_codes, _dm_guess_subject,
    _dm_affix_pattern,
)
# OCR text-recognition helpers (page titles, doc-type classify, name learning).
from apnescan_lib.ocr_text import (
    page_ocr_text, sig_words, _jaccard, _containment, learned_name_for,
...
# Give the Smart Orientation engine the app's cached OCR-availability check, so
# it uses the exact same result as the rest of the app (no behaviour drift) while
# staying free of any import back into this file.
_orientation.tesseract_available = tesseract_available
...
DEFAULT_STATS_URL = "https://status.apnesoft.com/stats.php"
...
# Image-processing engine moved to apnescan_lib/imaging.py (imported at top).


# Smart Orientation engine moved to apnescan_lib/orientation.py (imported at top).
...
# Fingerprint engine (_dm_* helpers) moved to apnescan_lib/fingerprint.py (imported at top).


# Intelligent Naming Engine moved to apnescan_lib/naming_engine.py (imported at top).
...
class DocMemory(object):
    ...
    def _init_schema(self):
        ...
        CREATE TABLE IF NOT EXISTS corrections(wrong TEXT PRIMARY KEY, correct TEXT, count INTEGER DEFAULT 1);
        """)
        try:
    ...
    def style_for(self, doc_type):
        """Learn the user's separator/order from past filenames of this type."""
        try:
            if doc_type:
                rows = self._db.execute("SELECT filename FROM docs WHERE doc_type=? AND filename!='' "
                                        "ORDER BY last_used DESC LIMIT 100", (doc_type,)).fetchall()
            else:
                rows = self._db.execute("SELECT filename FROM docs WHERE filename!='' "
                                        "ORDER BY last_used DESC LIMIT 100").fetchall()
            return _ni_learn_style([r["filename"] for r in rows])
        except Exception:
            return _ni_default_style()

    def corrections(self):
        """Learned OCR corrections {wrong: correct} (most-used first)."""
        try:
            return {r["wrong"]: r["correct"] for r in
                    self._db.execute("SELECT wrong, correct FROM corrections ORDER BY count DESC LIMIT 500").fetchall()}
        except Exception:
            return {}

    def add_correction(self, wrong, correct):
        if not self.ok() or not wrong or not correct or wrong.lower() == correct.lower():
            return
        with self._lock:
            try:
                self._db.execute(
                    "INSERT INTO corrections(wrong,correct,count) VALUES(?,?,1) "
                    "ON CONFLICT(wrong) DO UPDATE SET correct=?, count=count+1",
                    (wrong.lower(), correct, correct))
                self._db.commit()
            except Exception:
                pass
...
class ScannerWindow(QtWidgets.QMainWindow):
    ...
    def show_analytics(self):
        ...
        # (Worldwide/World tab hata diya — wo data sirf admin panel me,
        #  status.apnesoft.com/admin par login karke dikhta hai. App me sirf
        #  aapki apni stats.)
        ...
        # (Worldwide data app me nahi dikhta — wo sirf admin panel me.)
        self._an_update_banner()

        # ---- footer: sirf OK (worldwide/refresh hata diya) ----
        foot = QtWidgets.QHBoxLayout()
        bok = QtWidgets.QPushButton("OK"); bok.clicked.connect(dlg.accept)
        foot.addStretch(1); foot.addWidget(bok)
        top.addLayout(foot)
        note = QtWidgets.QLabel(L(
            "<span style='color:#94a3b8;font-size:11px;'>Ye stats sirf is PC ki hain. "
            "Worldwide ginti sirf admin panel me (status.apnesoft.com/admin).</span>",
            "<span style='color:#94a3b8;font-size:11px;'>These stats are for this PC only. "
            "Worldwide counts are in the admin panel (status.apnesoft.com/admin).</span>"))
        note.setTextFormat(QtCore.Qt.RichText); note.setWordWrap(True); top.addWidget(note)
        dlg.exec_()
...
"""
    ...
    def _build_ui(self):
        ...
        # PagesList drives the reorder drag itself (manual QDrag) — so the list is
        # drop-only; this is what makes mouse reorder actually work in IconMode.
        ...
        self.foot_online.hide()   # worldwide app me nahi — sirf admin panel me
    ...
    def _apply_ui_live(self):
        ...
        for attr, key in (("foot_folder", "footer_folder"), ("foot_pages", "footer_pages"),
            ...
            try:
                ...
                pass
        # worldwide-online footer hamesha chhupa (app me worldwide nahi dikhta)
        try:
            self.foot_online.hide()
        except Exception:
            pass
        try:
    ...
    def _ai_features(self, paths, filename="", folder="", file_size=0, page_count=0):
        ...
        try:
            ...
            try:
                # Intelligent Naming Engine category (Aadhaar/PAN/Invoice/…);
                # fall back to the older classifier if it can't decide.
                lbl, _tc = _ni_detect_type(txt)
                if lbl in ("Document", "Medical Document"):
                    lbl = classify_from_text(txt) or lbl
                f["doc_type"] = lbl
            except Exception:
                try:
    ...
    def _ai_try_pattern(self, res):
        """Intelligent Naming Engine for a never-seen document: builds a clean
        '<Name> - <Type>' filename from OCR (person/business + document category +
        number), in the user's learned style — even with no prior history.
        Prefers the exact learned pattern for this type when it exists.
        Only changes the default name; never auto-saves a guess."""
        ...
        try:
            ...
            sug = ""
            try:
                # 1) exact learned pattern for this type (keeps the user's own label)
                sug = self._memory.pattern_suggest(dt, txt)
            except Exception:
                sug = ""
            if not sug:
                # 2) generate from scratch using the naming engine + learned style/fixes
                try:
                    style = self._memory.style_for(dt)
                    corr = self._memory.corrections()
                except Exception:
                    style, corr = None, None
            ...
            if sug:
                ...
                try:
                    self.status.showMessage(self.L("💡 Sujhaya naam: %s", "💡 Suggested name: %s") % sug, 5000)
                except Exception:
                    pass
        except Exception:
            pass
    ...
    def _ai_remember_saved(self, paths, out, npages):
        ...
        try:
            ...
            # Auto-correction learning: if the name the AI extracted differs from
            # the name the user saved by a small OCR-typo (e.g. Kurnar -> Kumar),
            # remember the fix so it's applied automatically next time.
            if self._opts.get("ai_learning", True):
                try:
                    subj = getattr(self, "_doc_ai_subject", "") or ""
                    if subj:
                        for wrong, right in _ni_learn_corrections(subj, base.replace("_", " ")):
